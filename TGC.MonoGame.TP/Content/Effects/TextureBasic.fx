...
VertexShaderOutput BaseTextureVS(in VertexShaderInput input)
{
    VertexShaderOutput output;
    output.Position = mul(input.Position, WorldViewProjection);

    // Propagate scaled Texture Coordinates
    output.TextureCoordinate = input.TextureCoordinate * 1;

    return output;
}

float4 BaseTexturePS(VertexShaderOutput input) : COLOR
{
...
WorldVertexShaderOutput WorldTextureVS(in WorldVertexShaderInput input)
{
...
float4 WorldTexturePS(WorldVertexShaderOutput input) : COLOR
{
    ...
    float2 textureCoordinates = resultPlane * 1;
...
technique BaseTexture
{
    pass P0
    {
        VertexShader = compile VS_SHADERMODEL BaseTextureVS();
        PixelShader = compile PS_SHADERMODEL BaseTexturePS();
    }
};

technique WorldTexture
{
    pass P0
    {
        VertexShader = compile VS_SHADERMODEL WorldTextureVS();
        PixelShader = compile PS_SHADERMODEL WorldTexturePS();
    }
};
